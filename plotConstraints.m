...
for k = 1:length(d)
    for j = 1:dpm(months(k))
        v(sum(dpm(months(1:k-1)))+j) = V0+1e4*(sum(e(1:k-1))-sum(d(1:k-1))+j/dpm(months(k))*(e(k)-d(k)));
    end
end


for idx = 1:length(d)
    thisMonth = v(sum(dpm(months(1:idx))));
...
figure;
plot(0:sum(dpm(months(1:length(d)))), [V0 v], 'LineWidth', 2);
...
axis([0, sum(dpm(months(1:length(d)))), lowest-0.1*highest, 1.5*highest]);

set(gca,'fontsize',14)
hold on;

stairs([0 cumsum(dpm(months(1:length(d))))], [L(1:length(d)-1) Vn Vn], 'Color', 'red', 'LineStyle', '--');
plot(cumsum(dpm(months(1:length(d)-1))), L(1:length(d)-1), '.', 'MarkerSize', 20);

stairs([0  cumsum(dpm(months(1:length(d))))], [U(1:length(d)-1) Vn Vn], 'Color', [0 0.5 0], 'LineStyle', '--', 'LineWidth', 1.5);
plot(cumsum(dpm(months(1:length(d)-1))), U(1:length(d)-1), '.', 'MarkerSize', 20);

plot(0,V0,'.', 'MarkerSize', 20);
plot(length(v),Vn,'.', 'MarkerSize', 20);
legend({'Inventory','End of Month Inventory Minimum','Inventory Minimum',...
        'End of Month Inventory Maximum','Inventory Maximum',...
        'V_0','V_n'}, 'Location', 'northeast');
...
axis([1, sum(dpm(months(1:length(d)))), -1.25*max(w(1:length(d))), 1.5*max(i(1:length(d)))]);
xlabel('Day', 'fontsize', 18)
ylabel('Change in Inventory (mmbtu/day)','fontsize', 18);
hold on;

stairs([1 cumsum(dpm(months(1:length(d))))], [vd vd(end)], 'LineWidth', 2);
stairs([1 cumsum(dpm(months(1:length(d))))], [i(1:length(d)) i(length(d))], 'Color', [0 0.5 0], 'LineStyle', '--');
stairs([1 cumsum(dpm(months(1:length(d))))], [-w(1:length(d)) -w(length(d))], 'Color', 'red', 'LineStyle', '--');
